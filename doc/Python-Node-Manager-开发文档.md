...
用户流量由 Clash API 定时采样并累计到本地状态文件。来源 IP 在最近的活跃窗口内计为在线设备，默认窗口为 60 秒，避免短连接造成在线设备数抖动。达到流量额度或超过来源 IP 数量限制时，Node Manager 会写入 sing-box 拒绝规则并关闭已有连接；客户端不能通过立即重连绕过限制。提高或取消额度后，下一次采样会自动撤销对应拒绝规则。
